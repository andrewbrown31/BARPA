...
while [ "$d" != 2050-01-01 ]; do
  ...
  echo "INFO: RUNNING WRFPYTHON ON DATA FROM" $start_time "to" $end_time
  python /home/548/ab4502/working/ExtremeWind/wrf_non_parallel.py -m barpa -r aus -t1 $start_time -t2 $end_time -e rcp85 -barpa_experiment cmip5 --ens r1i1p1 --issave True --outname barpa_access_append --params min --barpa_forcing_mdl ACCESS1-0  
  d=$(date -I -d "$d + 1 month")
